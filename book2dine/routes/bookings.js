...
router.post("/", (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ error: "You must be logged in to book a restaurant." });
    }
    ...
    // Ensure bookingDate is in the correct integer format (YYYYMMDD as integer)
    const formattedBookingDate = parseInt(bookingDate.replace(/-/g, '').substring(2)); // Convert to YYYYMMDD (int)
    ...
    db.get(checkAvailabilitySql, [restaurantId, formattedBookingDate], (err, row) => {
        ...
        const availableSeats = row.maxPlaces;
